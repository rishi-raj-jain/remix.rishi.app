          ...
          <div className="mt-10 flex w-full flex-col lg:mt-20 lg:w-2/3 lg:pr-10">
            {allPosts.map((item, _) => (
              <div key={_} className="mb-10 flex flex-col border-b pb-10 dark:border-gray-700">
                ...
                {item && item.content && (
                  <Link prefetch="intent" to={'/blog/' + item.slug} className={`mt-3 text-lg font-bold hover:underline sm:text-2xl`}>
                    {item.content.title}
                  </Link>
                )}
                {item && item.content && <span className={`mt-3 text-sm text-gray-700 line-clamp-2 dark:text-gray-400`}>{item.content.intro}</span>}
                {item && item.slug && (
                  <Link prefetch="intent" to={'/blog/' + item.slug} className={`mt-5 text-sm uppercase text-blue-500 hover:underline`}>
                    Read More &rarr;
                  </Link>
                )}
              </div>
